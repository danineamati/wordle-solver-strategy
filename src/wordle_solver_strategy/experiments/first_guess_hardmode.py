...
def sub_job(agent_cls, sub_guesses, answers, guesses):
    ...
    bin_table = get_bin_table_inline(guesses_numba, answers_numba, answers_char_counts)
...
def main() -> None:
    ...
    results.to_csv(
        experiments_csv_dir() / "first_guess_hardmode_results.csv", index=False
    )

    failed_words = results[(results["answer"] != results["final_guess"])][
        "first_guess"
    ].unique()
    filtered_results = results[~results["first_guess"].isin(failed_words)]

    summary = filtered_results.groupby(["agent", "first_guess"]).mean()

    summary = summary[~summary["n_guesses"].isnull()].sort_values("n_guesses")
    summary.to_csv(experiments_csv_dir() / "first_guess_hardmode_results_summary.csv")
